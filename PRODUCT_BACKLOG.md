...
| U2 | Submission panes (question/code/AI summary) are fixed proportions. | `.ide-split` is static CSS grid/flex; no resize lib in `package.json`. | **Mockup approved.** `react-resizable-panels`: **two** draggable dividers — Problem↔Solution (vertical) + Code↔Report (horizontal) — sizes persisted to localStorage, min widths so no pane collapses to zero; handles hidden when panels stack on mobile. | platform/web | M–L |
| CX1 | The app is barely responsive — breaks on smaller/varied screens. | Only **2 `@media` rules** in the entire `components.css`. Candidates take assessments on many devices. | **Mockup approved — candidate-only scope.** Candidate IDE fully responsive at phone width (stack panels, wrap `.ide-top`, wrap `.actionbar` with full-width Submit, `.ide` height auto so it scrolls); interviewer pages get `.page-head` stacking + already-scrollable tables, treated desktop-first (**no** mobile nav this pass). | platform/web | M |
